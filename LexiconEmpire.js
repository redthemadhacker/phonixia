...
export default function LexiconEmpire({ username, email, onScoreUpdate, onBack }) {
  const [gameProgress, setGameProgress] = useState({
    game1: 0, // Dock & Sail Phonics Match
    game2: 0, // Floating Library Blending
    game3: 0, // Grand Bridge Sound Beats
    game4: 0, // Empire Courtyard Vowels
    game5: 0, // Observatory Spelling Unjumble
    game6: 0, // Crown Citadel Rhyme Tap
  });
  ...
  const loadLandProgress = async () => {
    try {
      ...
      let loadedGames = {};
      for (let i = 1; i <= 6; i++) {
        let val = await AsyncStorage.getItem(`@phonixia_${cleanParent}_${cleanExplorer}_lexiconEmpire_game${i}`);
        loadedGames[`game${i}`] = val ? parseInt(val, 10) : 0;
      }
      setGameProgress(loadedGames);
    } catch (e) {
      console.error('Failed to load Lexicon Empire progress', e);
    }
  };
  ...
  const saveGameScore = async (gameKey, newScore) => {
    try {
      const cleanParent = email.trim().toLowerCase();
      const cleanExplorer = username.trim();
      await AsyncStorage.setItem(`@phonixia_${cleanParent}_${cleanExplorer}_lexiconEmpire_${gameKey}`, newScore.toString());
      setGameProgress(prev => ({ ...prev, [gameKey]: newScore }));
      const gameIndex = gameKey.replace('game', '');
      onScoreUpdate('lexiconEmpire', gameIndex, newScore);
    } catch (e) {
  ...
  const generateKidFriendlyChallenge = (gameKey, level) => {
    ...
    switch (gameKey) {
      case 'game1': {
        const items = [
          { label: 'Sailboat', emoji: '⛵', sound: 's', correct: true },
          { label: 'Apple', emoji: '🍎', sound: 'a', correct: false },
          { label: 'Dog', emoji: '🐶', sound: 'd', correct: false },
        ].sort(() => Math.random() - 0.5);
        newChal = {
          promptText: "Find the dock item starting with the 'S' sound!",
          spokenText: "Find the item that starts with the s sound. Sailboat!",
          options: items,
        };
        break;
      }
      case 'game2': {
        newChal = {
          ...
          spokenText: "Listen. B... o... o... k... What word is it?",
          options: [
      ...
      case 'game3': {
        newChal = {
          promptText: "How many sound beats in 'EMPIRE'? Tap the count!",
          spokenText: "How many sound beats in empire? Em-pire. Count them!",
          options: [
            { label: '1 Beat', emoji: '1️⃣', correct: false },
            { label: '2 Beats (EM-PIRE)', emoji: '🏛️🏛️', correct: true },
      ...
      case 'game4': {
        newChal = {
          promptText: "What middle vowel sound do you hear in W-R-I-T-E?",
          spokenText: "What middle sound do you hear in write? W... r... i... t... e.",
          options: [
      ...
      case 'game5': {
        const wordList = ['WORD', 'PAGE', 'READ', 'TOME', 'INKY', 'QUILL'];
        ...
        newChal = {
          ...
          spokenText: `Spell the empire word ${target}!`,
          targetWord: target,
      ...
      case 'game6': {
        newChal = {
          promptText: "Which word rhymes with 'BOOK'?",
          spokenText: "Which word rhymes with book?",
          options: [
            { label: 'Cook', emoji: '🍳', correct: true },
  ...
  const handleSuccessfulAnswer = () => {
    setFeedback('🌟 Brilliant scholarship! Awesome job!');
    ...
    setTimeout(() => {
      if (currentLevel < 25) {
        ...
        Alert.alert("🎉 Empire Mastered!", "You finished all 25 levels in this Lexicon Empire zone!");
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.topBar}>
          ...
          <Text style={styles.explorerTag}>👑 Lexicon Empire | {username}</Text>
        </View>

        {/* 6 Landmark Nodes */}
        <View style={styles.nodesContainer}>
          <TouchableOpacity 
            style={[styles.gameNode, { bottom: '15%', left: '12%' }]} 
            onPress={() => openGame('game1', '⛵ Dock & Sail Phonics')}
          >
            <View style={styles.nodeCircle}><Text style={styles.nodeEmoji}>⛵</Text></View>
            <Text style={styles.nodeText}>Dock & Sail</Text>
          </TouchableOpacity>

          <TouchableOpacity 
            style={[styles.gameNode, { top: '28%', left: '15%' }]} 
            onPress={() => openGame('game2', '📖 Floating Library Blending')}
          >
            <View style={styles.nodeCircle}><Text style={styles.nodeEmoji}>📖</Text></View>
            <Text style={styles.nodeText}>Floating Library</Text>
          </TouchableOpacity>

          <TouchableOpacity 
            style={[styles.gameNode, { bottom: '42%', left: '33%' }]} 
            onPress={() => openGame('game3', '🌉 Grand Bridge Sound Beats')}
          >
            <View style={styles.nodeCircle}><Text style={styles.nodeEmoji}>🌉</Text></View>
            <Text style={styles.nodeText}>Grand Bridge</Text>
          </TouchableOpacity>

          <TouchableOpacity 
            style={[styles.gameNode, { bottom: '34%', left: '55%' }]} 
            onPress={() => openGame('game4', '🏛️ Empire Courtyard Vowels')}
          >
            <View style={styles.nodeCircle}><Text style={styles.nodeEmoji}>🏛️</Text></View>
            <Text style={styles.nodeText}>Courtyard</Text>
          </TouchableOpacity>

          <TouchableOpacity 
            style={[styles.gameNode, { top: '35%', right: '16%' }]} 
            onPress={() => openGame('game5', '🔭 Observatory Spelling Unjumble')}
          >
            <View style={styles.nodeCircle}><Text style={styles.nodeEmoji}>🔭</Text></View>
            <Text style={styles.nodeText}>Observatory</Text>
          </TouchableOpacity>

          <TouchableOpacity 
            style={[styles.gameNode, { top: '15%', left: '9%' }]} 
            onPress={() => openGame('game6', '🏰 Crown Citadel Rhyme Tap')}
          >
            <View style={styles.nodeCircle}><Text style={styles.nodeEmoji}>🏰</Text></View>
            <Text style={styles.nodeText}>Crown Citadel</Text>
          </TouchableOpacity>
        </View>
            ...
            <View style={styles.modalContent}>
              ...
              {activeGameKey === 'game5' ? (
                <View style={styles.unjumbleContainer}>
                  ...
                  <Text style={styles.subInstruction}>Tap the imperial letters in order:</Text>
...
const styles = StyleSheet.create({
  ...
  explorerTag: { color: '#fb8500', fontWeight: 'bold', fontSize: 13 },
  nodesContainer: { flex: 1, position: 'relative' },
  gameNode: { position: 'absolute', alignItems: 'center' },
  nodeCircle: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#3a86ff', justifyContent: 'center', alignItems: 'center', borderWidth: 3, borderColor: '#ffd166', elevation: 6 },
  ...
  modalContent: { width: '100%', maxWidth: 400, backgroundColor: '#1b263b', borderRadius: 20, padding: 20, alignItems: 'center', borderWidth: 2, borderColor: '#3a86ff' },
  ...
  visualOptionCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1d3557', padding: 14, borderRadius: 14, marginBottom: 10, width: '100%', borderWidth: 2, borderColor: '#3a86ff' },
  ...
  letterSlot: { width: 50, height: 50, backgroundColor: '#0d1b2a', borderWidth: 2, borderColor: '#3a86ff', borderRadius: 10, justifyContent: 'center', alignItems: 'center', marginHorizontal: 6 },
  ...
  treasureLetterCard: { width: 52, height: 52, backgroundColor: '#3a86ff', borderRadius: 12, justifyContent: 'center', alignItems: 'center', margin: 6, borderWidth: 2, borderColor: '#fff', elevation: 4 },
  treasureLetterText: { color: '#fff', fontSize: 24, fontWeight: 'bold' },
