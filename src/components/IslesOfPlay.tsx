...
export const IslesOfPlay: React.FC<IslesOfPlayProps> = ({ onBackToWorld }) => {
  ...
  // Filter for Isles of Play minigames (13 games)
  const islesGames = ALL_MINIGAMES.filter(g => g.hub === 'isles-of-play');
  ...
  const handleCheckAnswer = () => {
    ...
    if (correct) {
      ...
      recordMinigameScore('bubblePopper', score + activeGame.scoreReward);
    } else {
      sounds.playError();
    }
  };
  ...
  const handleNextGame = () => {
    ...
    const currentIndex = islesGames.findIndex(g => g.id === activeGame.id);
    const nextGame = islesGames[(currentIndex + 1) % islesGames.length];
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-6">
      {/* Game HUD Header */}
      <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-teal-900/60 via-emerald-900/60 to-slate-900 border-2 border-emerald-500/40 shadow-2xl flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-xs font-bold uppercase tracking-wider text-emerald-400">
              Floating Playground · 13 Minigames
            </div>
      ...
      {/* Main Grid: 13 Minigames */}
      {!activeGame ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {islesGames.map((game, index) => (
            <div
              key={game.id}
              className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-emerald-400/50 transition-all flex flex-col justify-between group"
            >
              <div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between">
                <span className="text-xs font-mono text-amber-400 font-bold">
                  +{game.scoreReward} pts
                </span>
                <button
                  onClick={() => handleLaunchGame(game)}
                  className="px-4 py-1.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-bold text-xs uppercase tracking-wider shadow-md transition-all cursor-pointer flex items-center gap-1.5"
                >
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-slate-900 border-2 border-emerald-500/50 shadow-2xl space-y-6 max-w-3xl mx-auto">
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            ...
            >
              <ArrowLeft className="w-4 h-4" />
              <span>Back to Isles Hub</span>
            </button>
            <div className="flex items-center gap-3">
              <button
                onClick={() => sounds.speak(activeGame.prompt)}
                className="px-3 py-1.5 rounded-lg bg-emerald-500/20 text-emerald-300 text-xs font-semibold flex items-center gap-1.5 cursor-pointer"
              >
                <Volume2 className="w-4 h-4" />
                <span>Repeat Voice</span>
              </button>
              <span className="text-xs font-mono text-amber-400 font-bold">
                +{activeGame.scoreReward} pts
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3.5 max-w-lg mx-auto">
            {activeGame.options.map((option, idx) => {
              ...
              let style = 'bg-slate-800 border-slate-700 hover:border-emerald-400/60 text-slate-200';
              ...
              if (hasChecked) {
                ...
                style = 'bg-emerald-500/20 border-emerald-400 text-emerald-200 scale-102';
              }
            ...
            >
              ...
              <div className="space-y-1">
                <div className="text-xs font-bold uppercase tracking-wider">
                  {isCorrect ? 'Well Done!' : 'Listen & Learn'}
                </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
            {!hasChecked ? (
              <button
                ...
                className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 disabled:opacity-40 text-slate-950 text-xs font-black uppercase tracking-wider shadow-lg transition-all cursor-pointer flex items-center gap-2"
              >
